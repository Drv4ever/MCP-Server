...
server.tool(
  "run_command",
  "Executes a terminal command (like npm install or mkdir) in the project directory",
  {
    command: z
      .string()
      .describe(
        "The shell command to run, e.g. 'npm install' or 'mkdir myproject'",
      ),
...
server.tool(
  "setupdatabase",
  "Creates a MongoDB connection file (db.js) with boilerplate code",
  {
...
server.tool(
  "setup_express",
  "Generates a boilerplate Express server.js file with CORS, dotenv, and DB connection",
  {
    projectPath: z.string().describe("The absolute path of your project root"),
  },
  async ({ projectPath }) => {
...
server.tool(
  "setup_model",
  "Creates a standard Mongoose model file in a /models directory",
  {
    projectPath: z.string().describe("The absolute path of your project root"),
    modelName: z
      .string()
